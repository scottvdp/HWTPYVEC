...
if sys.version_info[0] == 3:
  def ordat(b, i): return b[i]
else:
  def ordat(b, i): return ord(b[i])
    ...
    if w == b'stream':
      ...
      if m:
        ...
        if streamend > 0:
          ...
          return ((OSTREAM, (o[1], streamstart, streamend)), streamend+9)
    return (o, j)
  return (None, i+1)
...
def GetPDFHexString(s, i, iend):
  """Convert and return pdf hex string starting at s[i], ending at s[iend-1]."""
...
def GetPDFTrailerAndCrossrefs(s):
  """Find and return the (last) PDF trailer dictionary and cross reference dict.
...
def ReadPDFPageOneContents(filename):
  ...
  try:
    f = open(filename, "rb")  # need binary since some parts may be compressed
  except IOError:
    if WARN:
...
def GetPDFPageOneContents(s):
  ...
  while pnode:
    ...
    if pnodetype == 'Pages':
      kidsarray = GetTypedValFromDictEntry(pnode, 'Kids', OARRAY, s, crossrefs)
      if not kidsarray:
        if WARN:
      ...
      contentsobj = GetPDFObjFromDictEntry(pnode, 'Contents', s, crossrefs)
      if contentsobj is None:
        # it is legal for there to be no contents object: means empty page
        if WARN:
      ...
      if contentsobj[0] == OSTREAM:
        ...
        if WARN:
          print('Contents object has unexpected type', contentsobj[0])
        return ''
    else:
      if WARN:
