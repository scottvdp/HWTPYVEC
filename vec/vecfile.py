...
def TokenizeAIEPSFile(filename):
  ...
  try:
    f = open(filename, "rU")  # 'U' means all newline reps converted to '\n'
  except IOError:
    if WARN:
...
class _PathState(object):
  ...
  def CloseSubpath(self):
    ...
    Close the current subpath by appending a straight line segment from current
    point to starting point of the subpath, terminating current subpath.
  ...
  def DrawPath(self, dofill, dostroke, fillevenodd=False):
    ...
    Arguments:
      ...
      fillevenodd: it true, use even-odd fill rule, else nonzero winding number rule
    """
  ...
  while i < len(toks):
    ...
    if t == TNAME:
      ...
      if v == "h" or v == "H" or v == "closepath":
        ...
        # finish path no-op, probably after clipping (which is not handled yet)
        pstate.ResetPath()
        ...
        if len(args) == 3:
          ...
          if op == "v" or op == "V":
            ...
            pstate.Bezier3To(args[2], args[3], 0.0, 0.0,
                             args[0], args[1], use_start_as_cp=True)
          elif op == "y" or op == "Y":
